...
class GameTestCase(chex.TestCase):
    ...
    def test_update_trajectories_step_0_preserves_first_state(self):
        trajectories = game.update_trajectories(self.linear_go_model, self.params,
                                                rng_key=jax.random.PRNGKey(42), step=0,
                                                trajectories=game.new_trajectories(
                                                    board_size=self.board_size, batch_size=1,
                                                    trajectory_length=6))

        np.testing.assert_array_equal(trajectories.nt_states[:, 0],
                                      jnp.zeros_like(trajectories.nt_states[:, 0]))

    def test_update_trajectories_step_0_updates_first_action_and_second_state(self):
        trajectories = game.update_trajectories(self.linear_go_model, self.params,
                                                rng_key=jax.random.PRNGKey(42), step=0,
                                                trajectories=game.new_trajectories(
                                                    board_size=self.board_size, batch_size=1,
        ...
        np.testing.assert_array_equal(trajectories.nt_states[:, 1], gojax.decode_states("""
                                                        _ _ B
                                                        _ _ _
                                                        _ _ _
                                                        TURN=W
    ...
    def test_update_trajectories_step_1_preserves_first_two_states_and_first_action(self):
        updated_data = game.update_trajectories(self.linear_go_model, self.params,
    ...
    def test_update_trajectories_step_1_updates_second_action_and_third_state(self):
        updated_data = game.update_trajectories(self.linear_go_model, self.params,
                                                rng_key=jax.random.PRNGKey(42), step=1,
                                                trajectories=game.new_trajectories(
                                                    board_size=self.board_size, batch_size=1,
    ...
    def test_rotationally_augments_states_on_8x1_trajectory_with_3x3_board(self):
        states = gojax.decode_states("""
                                    B _ _
                                    _ _ _
                                    _ _ _
                                    """)
        nt_states = jnp.repeat(nt_utils.unflatten_first_dim(states, 1, 1), axis=0, repeats=8)
        ...
        expected_rot_aug_states = gojax.decode_states("""
                                                    ...
                                                    _ _ _
                                                    """)
        expected_rot_aug_nt_states = nt_utils.unflatten_first_dim(expected_rot_aug_states, 8, 1)
        filler_nt_actions = jnp.zeros((8, 1), dtype='uint16')
        rot_traj = game.rotationally_augment_trajectories(
            game.Trajectories(nt_states=nt_states, nt_actions=filler_nt_actions))
        np.testing.assert_array_equal(rot_traj.nt_states, expected_rot_aug_nt_states)

    def test_rotationally_augments_actions_on_8x1_trajectory_with_3x3_board(self):
        nt_actions = jnp.zeros((8, 1), dtype='uint16')
        expected_nt_actions = jnp.array([[0], [0], [6], [6], [8], [8], [2], [2]], dtype='uint16')
        filler_nt_states = nt_utils.unflatten_first_dim(
