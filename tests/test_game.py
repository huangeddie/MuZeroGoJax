...
class GameTestCase(chex.TestCase):
    ...
    def setUp(self):
        self.absl_flags.board_size = 3
        main.FLAGS(
            f'foo --board_size={self.absl_flags.board_size} --embed_model=identity --value_model=random '
    ...
    def test_new_trajectories(self):
        new_trajectories = game.new_trajectories(board_size=self.absl_flags.board_size, batch_size=2,
                                                 max_num_steps=9)
    ...
    def test_random_sample_next_states_3x3_42rng(self):
        ...
        next_states = game.sample_next_states(self.random_go_model, params={},
                                              ...
                                              states=gojax.new_states(board_size=self.absl_flags.board_size))
    ...
    def test_update_trajectories_step_0(self):
        trajectories = game.new_trajectories(board_size=self.absl_flags.board_size, batch_size=1,
                                             max_num_steps=6)
    ...
    def test_random_self_play_3x3_42rng(self):
        ...
        expected_trajectories = gojax.decode_states("""
                                                    ...
                                                    _ W B
                                                    """)
        expected_trajectories = jnp.reshape(expected_trajectories, (1, 3, 6, 3, 3))

        def _get_trajectory_pretty_string(trajectories, index=0):
            pretty_trajectory_str = '\n'.join(
                map(lambda state: gojax.get_string(state), trajectories[index]))
    ...
    def test_get_winners_one_tie_one_winning_one_winner(self):
        trajectories = game.new_trajectories(board_size=self.absl_flags.board_size, batch_size=3,
                                             max_num_steps=2)
    ...
    def test_get_actions_and_labels_with_komi(self):
        sample_trajectory = gojax.decode_states("""
                                            ...
                                            TURN=W
                                            """)
        actions, labels = game.get_actions_and_labels(
    ...
    def test_get_actions_and_labels_multi_kill(self):
        sample_trajectory = gojax.decode_states("""
                                            ...
                                            """)
        actions, labels = game.get_actions_and_labels(
            jnp.reshape(sample_trajectory, (1, 2, 6, 3, 3)))
